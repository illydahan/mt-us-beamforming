...
from cubdl.PlaneWaveData import PICMUSData, PlaneWaveData


def load_data(data_source, acq, target = None, full_dir = None):
    ...
    if data_source == "MYO":
        ...
        xlims = [P.ele_pos[0, 0], P.ele_pos[-1, 0]]
        zlims = [0e-3, 30e-3]
    elif data_source == "PIC":
        P = PICMUSData(database_path, acq, 'contrast_speckle', 'iq')
        xlims = [P.ele_pos[0, 0], P.ele_pos[-1, 0]]
        zlims = [5e-3, 55e-3]
    else:
...
class USTBData(PlaneWaveData):
    ...
    def __init__(self, database_path, acq):
        ...
        self.ele_pos = np.array(f["probe_geometry"], dtype="float32")#np.stack([xpos, 0 * xpos, 0 * xpos], axis=1)
