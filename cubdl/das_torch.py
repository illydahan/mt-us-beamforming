...
class PW_LOADER():
    ...
    def tof_interpolate(self, x):
        ...
        # itof = torch.zeros(nangles, nelems, npixels, dtype=self.dtype, device=self.device)
        # qtof = torch.zeros(nangles, nelems, npixels, dtype=self.dtype, device=self.device)
        itof = torch.zeros(nangles, nelems, npixels, dtype=self.dtype, device=self.device)
        qtof = torch.zeros(nangles, nelems, npixels, dtype=self.dtype, device=self.device)
        
        
        
        for angle_idx, (t, td) in tqdm(enumerate(zip(self.ang_list, txdel)), total=len(self.ang_list)):
            
            for ele_idx, (r, rd) in enumerate(zip(self.ele_list, rxdel)):
                ...
                itof[angle_idx, ele_idx] = ifoc.detach().cpu()
                qtof[angle_idx, ele_idx] = qfoc.detach().cpu()
                
               
                
                #qtof[angle_idx, ele_idx] = qfoc
            
            # itof.append(itof_angle)
            # qtof.append(qtof_angle)
...
class DAS_PW(torch.nn.Module):
    ...
    def forward(self, x, return_tof_corr = False, apod=True, angle_wise = False, regular = True):
        ...
        if return_tof_corr or angle_wise:
            itof = []
            qtof = []
        # Loop over angles and elements
        for r, rd, ra in zip(self.ele_list, rxdel, rxapo):    
            curr_idas = torch.zeros(npixels, dtype=self.dtype, device=self.device)
            curr_qdas = torch.zeros(npixels, dtype=self.dtype, device=self.device)
            ...
            for t, td, ta in zip(self.ang_list, txdel, txapo):
                ...
                if not apod:
                    apods = torch.ones_like(apods)
                    
                idas += ifoc * apods
                qdas += qfoc * apods
                
                curr_idas += ifoc * apods
                curr_qdas += qfoc * apods
                
                if angle_wise:
                    itof_angle.append(ifoc.detach().cpu().numpy())
                    qtof_angle.append(qfoc.detach().cpu().numpy())
                else:
                    itof_angle = curr_idas
                    qtof_angle = curr_qdas
            ...
            if angle_wise:
                ...
                itof_angle, qtof_angle = np.array(itof_angle), np.array(qtof_angle)
                if isinstance(itof, list):
                    itof = np.zeros_like(itof_angle)
                    qtof = np.zeros_like(qtof_angle)
                itof += itof_angle
                qtof += qtof_angle
        ...
        if angle_wise:
            itof, qtof = np.array(itof), np.array(qtof)
            tof = torch.stack([torch.from_numpy(itof), torch.from_numpy(qtof)])
            return idas, qdas, tof
        
        if return_tof_corr:
            itof, qtof = torch.stack(itof), torch.stack(qtof)
            tof = torch.stack([itof, qtof])
            return idas, qdas, tof
